...
func Menu(opt wmenu.Opt) (err error) {
	...
	ok = true
	fmt.Println("This will disable soulbinders by removing their spawnentry and spawngroups")
...
// Clean removes soulbinder
func Clean(c *client.Client) (err error) {
	ids, err := c.Database.SpawnGroupIDsByNameWildcardSearch(focus)
